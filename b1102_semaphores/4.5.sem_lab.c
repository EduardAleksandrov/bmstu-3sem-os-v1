/*
    ...
    устанавливаем начальные значения для семафоров - работает
*/
#define _GNU_SOURCE 
#include <stdio.h>
#include <stdlib.h>
...
int producer(int fd, char *data, char *startPos, int *start)
{
    ...
    while(flag)
    {
        ...
        *start += 1;
        ...
        // int sem_val = semctl(fd, 0, GETVAL, 0); //печать значения семафора для производителя
        // printf("\tсемафор prod:%d \n", sem_val);
        sleep(1);
...
int consumer(int fd, char *startPos, int *start, int *end)
{
    while(flag)
    {
        ...
        // int sem_val = semctl(fd, 0, GETVAL, 0);   //печать значения семафора для потребителя
        // printf("\tсемафор cons:%d \n", sem_val);
        sleep(1);
